...
def store_in_pinecone(id, text, metadata):
    """Store text in Pinecone with integrated embeddings using records API."""
    try:
        import requests
        ...
        # Use the records/upsert endpoint for integrated embeddings
        url = f"{pinecone_host}/records/upsert"
        headers = {
            "Api-Key": pinecone_key,
            "Content-Type": "application/json"
        }
        
        # Records format: _id, text (for embedding), plus any other fields as metadata
        record = {
            "_id": id,
            "text": text,  # This gets embedded automatically
            **metadata  # Additional fields stored as metadata
        }
        ...
        if response.status_code == 200:
            ...
            app.logger.error(f"Pinecone store error: {response.status_code} - {response.text}")
            return False
...
def search_pinecone(query, top_k=5, filter_dict=None):
    """Semantic search in Pinecone with integrated embeddings using records API."""
    try:
        import requests
        ...
        # Use the records/search endpoint for integrated embeddings
        url = f"{pinecone_host}/records/search"
        headers = {
            "Api-Key": pinecone_key,
            "Content-Type": "application/json"
        }
        ...
        if response.status_code == 200:
            data = response.json()
            # Convert to match format expected by rest of code
            matches = []
            for hit in data.get("result", {}).get("hits", []):
                class Match:
                    def __init__(self, hit):
                        ...
                        self.metadata = {k: v for k, v in hit.items() if not k.startswith("_")}
                matches.append(Match(hit))
            return matches
        else:
            app.logger.error(f"Pinecone search error: {response.status_code} - {response.text}")
            return []
...
def report_result():
    ...
    if success and user_input and code:
        # Store successful interaction in Pinecone for learning
        interaction_id = f"interaction_{hashlib.sha256((user_input + code).encode()).hexdigest()[:16]}"
        try:
            store_in_pinecone(
                id=interaction_id,
                text=user_input,
            ...
            app.logger.info(f"Stored successful interaction: {user_input[:50]}...")
        except Exception as e:
    ...
    return jsonify({"status": "recorded", "stored": success})
