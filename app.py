...
PINECONE_INDEX_NAME = os.environ.get('PINECONE_INDEX_NAME', 'murph-brain-v2')
PINECONE_HOST = os.environ.get('PINECONE_HOST', 'https://murph-brain-v2-or2vyhb.svc.aped-4627-b74a.pinecone.io')
...
def store_in_pinecone(id, text, metadata):
    ...
    try:
        ...
        if not index:
            app.logger.error("Pinecone index not available")
            return False
        
        # Record format per Pinecone docs
        record = {
            "_id": id,
            "text": text,  # Must match field_map in index config
            **metadata
        }
        
        # upsert_records(namespace, records) - namespace is positional
        index.upsert_records("__default__", [record])
        app.logger.info(f"Stored in Pinecone: {id}")
        return True
            
    except Exception as e:
        app.logger.error(f"Pinecone store error: {e}")
        return False
...
def search_pinecone(query, top_k=5, filter_dict=None):
    ...
    try:
        ...
        # Use index.search() per Pinecone docs
        search_query = {
            "top_k": top_k,
            "inputs": {"text": query}
        }
        ...
        app.logger.info(f"Pinecone search results: {results}")
        
        # Parse results per docs: results['result']['hits']
        matches = []
        for hit in results.get('result', {}).get('hits', []):
            class Match:
                def __init__(self, h):
                    self.id = h.get('_id')
                    self.score = h.get('_score')
                    self.metadata = h.get('fields', {})
            matches.append(Match(hit))
        
        return matches
            
    except Exception as e:
        app.logger.error(f"Pinecone search error: {e}")
        return []
...
def chat():
    ...
    app.logger.info(f"Chat request: {user_input[:100]}...")
    
    # Search for relevant knowledge
    relevant = search_pinecone(user_input, top_k=5)
    app.logger.info(f"Found {len(relevant)} relevant matches in Pinecone")
    ...
    for match in relevant:
        ...
        app.logger.info(f"  Match: {meta.get('type')} - score {getattr(match, 'score', 'N/A')}")
    
    context = "\n".join(context_parts) if context_parts else None
    
    context = "\n".join(context_parts) if context_parts else None
    
    # Get features for context - include full code templates
    features = Feature.query.filter_by(enabled=True).all()
    features_parts = []
    for f in features:
        methods_parts = []
        for m in f.methods:
            methods_parts.append(f"""  - {m.method_name}: {m.description or ''}
    Code ({m.code_type}):
    ```
{m.code_template}
    ```
    Examples: {', '.join(m.examples) if m.examples else 'N/A'}""")
        features_parts.append(f"Feature: {f.name} ({f.id})\n" + "\n".join(methods_parts))
...
Available features and their code templates:
{features_context}

User request: {user_input}

IMPORTANT: Return the FULL executable code in the "code" field, not just the method name.
Respond with JSON only."""
    ...
    try:
        ...
        # Default code_type to applescript if code exists but type missing
        if result.get('code') and not result.get('code_type'):
            result['code_type'] = 'applescript'
...
def report_result():
    ...
    response_text = data.get('response', '')  # Murph's spoken response
    
    # Store ALL interactions in Pinecone for memory/learning
    if user_input:
        interaction_id = f"interaction_{hashlib.sha256((user_input + str(datetime.utcnow().timestamp())).encode()).hexdigest()[:16]}"
        try:
            metadata = {
                'type': 'conversation',
            ...
            store_in_pinecone(
                id=interaction_id,
                text=user_input,  # Embed the user's input for semantic search
                metadata=metadata
            )
            app.logger.info(f"Stored interaction: {user_input[:50]}...")
        except Exception as e:
            app.logger.error(f"Failed to store in Pinecone: {e}")
    
    if not success and error:
        app.logger.warning(f"Execution failed: {error}")
    
    return jsonify({"status": "recorded", "stored": bool(user_input)})
