...
def store_in_pinecone(id, text, metadata):
    ...
    try:
        ...
        # The 'text' field must match the field_map configured in the index
        record = {
            "_id": id,
            "text": text,  # This gets auto-embedded based on field_map
            **metadata
        }
        ...
        for hit in results.get("result", {}).get("hits", []):
            class Match:
                def __init__(self, h):
                    ...
                    self.metadata = {k: v for k, v in h.items() if not k.startswith("_")}
